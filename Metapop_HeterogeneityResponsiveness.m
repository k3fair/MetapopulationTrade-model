%     Metapop_HeterogeneityEfficiency.m runs simulations with heterogeneous patch responsiveness
...
prop_high = 1; %specify proportion of patches with high gamma value (high efficiency)
...
gammavec_A = gammavec_temp(randperm(length(gammavec_temp))); %randomly distribute low and high gamma values
gammavec_I = gammavec_A;
...
dlmwrite(sprintf('PPlane_NOurban_betaA_betaI_WSnet_N%i_gamma_netexpHETERO%.2f_lowyield_export%i.csv',N, prop_high, eperc),storage_total, 'delimiter', ',', 'precision', 5);
...
b_A(1) = 1/(1+ exp(gammavec_A(1)*(beta_A- ((P(1))/(F(1))))));  %agricultural land expansion demand sigmoid
b_I(1) = 1/(1+ exp(gammavec_I(1)*(beta_I- ((P(1))/(F(1)))))); %food import demand sigmoid

for j = 2:N
    b_A(j) = 1/(1+ exp(gammavec_A(j)*(beta_A- ((P(j))/(F(j))))));
    b_I(j) = 1/(1+ exp(gammavec_I(j)*(beta_I- ((P(j))/(F(j))))));
end
