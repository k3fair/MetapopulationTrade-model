%     Metapop_HeterogeneityImportThreshold.m runs simulations with heterogeneous patch import demand thresholds
...
prop_high = 0.95; %specify proportion of patches with high beta_I value (low import consumption demand)
...
fstruct = dir(sprintf('WSnet_1_%iN_26_0.5_*.csv',N));

nsims=length(fstruct);
storage_total=zeros(length(tspan)*N*nsims,16);
...
%generate vectors of randomly ordered beta values
beta_low = 0.25;
beta_high = 1.25;
betavec_temp =  [beta_high*ones(1,round(N*prop_high)) beta_low*ones(1,round(N*(1-prop_high)))] ;
betavec_I = betavec_temp(randperm(length(betavec_temp)));
%since we still want same betaA values
betavec_A = 0.25*ones(1,N);
...
for i=1:length(Tic)
    for j=1:N
    b_A(i,j) = 1/(1+ exp(gamma_A*(betavec_A(j)- ((P(i,j))/(F(i,j))))));
    b_I(i,j) = 1/(1+ exp(gamma_I*(betavec_I(j)- ((P(i,j))/(F(i,j))))));
    end
end
...
storage_ts=zeros(length(tspan),16);
storage_run=zeros(length(tspan)*N,16);

counter_run=1;
for loop2=1:4:4*N

    for loop3=1:length(Tic)
            storage_ts(loop3,:) = [length(net(:,1)) neigh rewire gamma_I betavec_I(node) gamma_A betavec_A(node) node Tic(loop3) Ric(loop3,loop2) Ric(loop3,loop2+1) Ric(loop3,loop2+2) Ric(loop3,loop2+3) realization prop_high imports(loop3,node) ];
    end
...
dlmwrite(sprintf('PPlane_NOurban_betaA_betaI_WSnet_N%i_beta_netexpHETERO%.2f_lowyield_export%i.csv',N, prop_high, eperc),storage_total, 'delimiter', ',', 'precision', 5);
...
b_A(1) = 1/(1+ exp(gamma_A*(betavec_A(1)- ((P(1))/(F(1))))));  %agricultural land expansion demand sigmoid
b_I(1) = 1/(1+ exp(gamma_I*(betavec_I(1)- ((P(1))/(F(1)))))); %food import demand sigmoid

for j = 2:N
    b_A(j) = 1/(1+ exp(gamma_A*(betavec_A(j)- ((P(j))/(F(j))))));
    b_I(j) = 1/(1+ exp(gamma_I*(betavec_I(j)- ((P(j))/(F(j))))));
end
