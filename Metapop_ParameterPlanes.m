%     Metapop_ParameterPlanes.m runs parameter plane analysis
...
%%%Make scenario selections

N=100; % Specify number of patches to split IC between (10 or 100)
yield.select=2; %Specify whether you want to consider a low yield (=1) or a high yield (=2) scenario
experiment.select=2; %Specify whether you want to explore a (beta, gamma) plane (=1) or a (beta_A, beta_I) plane (=2)

%Read parameters in from fitting exercise (scenario: 70=high yield, 35=low yield)
if yield.select==1
    params = csvread('Ky_35_params_NOurban_Feb5.csv');
end
if yield.select==2
    params = csvread('Ky_70_params_NOurban_Feb5.csv');
end
...
fstruct = dir(sprintf('net_1_%iN*.csv',N));

nsims=500; %Select how many simulations you want to run
storage_total=zeros(length(tspan)*N*nsims,14);

loop1=1;
oops=0;
while loop1 < nsims+1

netselect=randi(length(fstruct));
net = csvread(fstruct(netselect).name,1); %the 1 indicates we skip the header row introduced by writing in R
file.string=strsplit(fstruct(netselect).name, '_');
neigh =str2double(file.string(4));
rewire=str2double(file.string(5));

if experiment.select==1
    gamma_I = 10*rand;
    beta_I = 2.5*rand;
    gamma_A=gamma_I;
    beta_A=beta_I;
...
storage_ts=zeros(length(tspan),14);
storage_run=zeros(length(tspan)*N,14);

counter_run=1;
for loop2=1:4:4*N

    for loop3=1:length(Tic)
            storage_ts(loop3,:) = [length(net(:,1)) neigh rewire gamma_I beta_I gamma_A beta_A node Tic(loop3) Ric(loop3,loop2) Ric(loop3,loop2+1) Ric(loop3,loop2+2) Ric(loop3,loop2+3) imports(loop3,node)];
    end
...
if (yield.select==1 && experiment.select==1)
dlmwrite(sprintf('PPlane_NOurban_betaA_gammaA_sWnet_N%i_maxbeta2_5_lowyield_export%itest.csv',N, eperc),storage_total, 'delimiter', ',', 'precision', 5);
end
if (yield.select==1 && experiment.select==2)
dlmwrite(sprintf('PPlane_NOurban_betaA_betaI_sWnet_N%i_gamma7_5_lowyield_export%itest.csv',N, eperc),storage_total, 'delimiter', ',', 'precision', 5);
end
if (yield.select==2 && experiment.select==1)
dlmwrite(sprintf('PPlane_NOurban_betaA_gammaA_sWnet_N%i_maxbeta2_5_highyield_export%itest.csv',N, eperc),storage_total, 'delimiter', ',', 'precision', 5);
end
if (yield.select==2 && experiment.select==2)
dlmwrite(sprintf('PPlane_NOurban_betaA_betaI_sWnet_N%i_gamma7_5_highyield_export%itest.csv',N, eperc),storage_total, 'delimiter', ',', 'precision', 5);
end
...
b_I(1) = 1/(1+ exp(gamma_I*(beta_I- ((P(1))/(F(1)))))); % import demand sigmoid
...
f=[ Y(1)*r_y*(1-(Y(1)/K_y)); %yield DE
    ...
    kappa*b_A(1)*(T_patch-A(1))-zeta*A(1); %agrilctulral land area DE
